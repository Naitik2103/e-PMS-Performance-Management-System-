...
(async () => {
  try {
    ...
    const ensureUser = async ({ first, last, email, role, deptId }) => {
      const exists = await pool.query("SELECT user_id FROM users WHERE LOWER(email) = $1 LIMIT 1", [email.toLowerCase()]);
      if (exists.rows.length) return exists.rows[0].user_id;
      const inserted = await pool.query(
        "INSERT INTO users (first_name, last_name, email, password_hash, role, department_id, is_active) VALUES ($1,$2,$3,$4,$5,$6,true) RETURNING user_id",
        [first, last, email.toLowerCase(), hash, role, deptId]
      );
    ...
    const revoId = await ensureUser({ first: "Riya", last: "Review", email: "reviewing@epms.local", role: "reviewing_officer", deptId: coDeptId });
    const roId = await ensureUser({ first: "Rohan", last: "Report", email: "reporting@epms.local", role: "reporting_officer", deptId: csDeptId });
    const empId = await ensureUser({ first: "Emma", last: "Employee", email: "employee@epms.local", role: "employee", deptId: csDeptId });
